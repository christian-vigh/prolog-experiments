...
%                                                                         %
% A L E P H                                                               %
% Version 5    (last modified: Sun Jun  4 10:51:31 UTC 2006)              %
%                                                                         %
...
aleph_version(5).
aleph_version_date('Sun Jun  4 10:51:31 UTC 2006').
...
expand(false,_,NodeRef,NodeRef,Path1,Length,Descendents,PCover,NCover,OVars,C,TV,CL):-
	!,
...
expand(_,S,NodeRef,NodeRef,Path1,Length,[_],PosCover,NegCover,OVars,_,_,_):-
        ...
        arg(5,S,Greedy),
        (Greedy = true ->
                '$aleph_global'(atoms_left,atoms_left(pos,PosCover));
                arg(16,S,PSize),
                PosCover = [1-PSize]),
        arg(4,S,_/Evalfn),
	(Evalfn = posonly -> 
                '$aleph_global'(atoms_left,atoms_left(rand,NegCover));
                arg(24,S,NSize),
                NegCover = [1-NSize]).
...
ask_best_split(Splits,Gain,Left,Right):-
	...
	(Answer = none ->
		...
		remove_nth(SplitNum,Splits,MCR-[Left,Right],_),
		Gain is -1*MCR
	),
	!.
...
induce_max1(Finish):-
        ...
        asserta('$aleph_global'(example_selected,example_selected(pos,Start))),
        sat(Start),
        reduce,
        update_coverset(pos,Start),
        retract('$aleph_global'(example_selected,example_selected(pos,Start))),
        Next is Start+1,
...
induce_features:-
	clean_up,
	store_values([good,check_good,updateback,construct_features]),
	set(good,true),
	set(check_good,true),
	...
	show(features),
        ...
        reinstate_values([good,check_good,updateback,construct_features]), !.
induce_features.
...
gen_features:-
	...
	feature(Id,(Template:-Body)),
	gen_feature_class(Id,PredictArg,Head,Body),
	fail.
gen_features:-
	(setting(dependent,PredictArg) -> true; PredictArg is 0),
	setting(good,true),
	...
	repeat,
	read(Stream,Fact),
	(Fact = '$aleph_good'(_,[_,_,_,F],Clause) ->
		F >= FMin,
		split_clause(Clause,Head,Body),
		Body \= true,
		functor(Head,Name,Arity),
		...
		copy_iargs(Arity,Head,Template,PredictArg),
        	gen_featurenum(Id),
        	assertz('$aleph_feature'(feature,feature(Id,Template,Body))),
		gen_feature_class(Id,PredictArg,Head,Body),
		fail;
		close(Stream), !
	).
...
gen_feature_class(Id,Argno,Head,Body):-
	has_class(Argno,Head,Body,Class),
	assertz('$aleph_feature'(feature_class,class(Id,Class))).
gen_feature_class(_,_,_,_).
...
gen_sample(Type,0):-
	...
	assertz('$aleph_global'(example_selected,example_selected(Type,
					ExampleNum))).
...
label_create(Clause,Type1,Set1,Type2,Set2,Label):-
	...
	nlits((Head,Body),Length),
        assertz('$aleph_search'(pclause,pclause(Head,Body))),
        '$aleph_global'(depth,set(depth,Depth)),
	('$aleph_global'(prooftime,set(prooftime,Time)) -> true; Time is inf),
...
features(Id,Feature):-
	'$aleph_feature'(feature,feature(Id,Template,Body)),
...
show(features):-
	...
	'$aleph_feature'(feature,feature(Id,Head,Body)),
        pp_dclause(feature(Id,(Head:-Body))),
	fail.
show(features):-
	'$aleph_feature'(feature_class,class(Id,Class)),
	pp_dclause(feature_class(Id,Class)),
	fail.
show(constraints):-
	setting(good,true),
	nl,
...
set_def(proof_strategy, miscellaneous,
	...
	[restricted_sld, sld], restricted_sld,
	show).
...
set_def(tries, search-search_strategy,
	'Number of restarts for ranomised search',
	int(1)-int(+inf), 10,
	show).
